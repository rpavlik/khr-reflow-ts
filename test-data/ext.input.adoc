...
When the application creates an slink:XrInstance
 object on Android systems,
additional information from the application has to be provided to the XR
runtime.

The Android XR runtime must: return error
...
elink:XrStructureType enumeration
is extended with:
...
* pname:next is code:NULL or a pointer
  to the next structure in a structure
  chain.
...
slink:XrInstanceCreateInfoAndroidKHR contains
